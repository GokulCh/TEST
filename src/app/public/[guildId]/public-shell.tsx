...
export function PublicShell({ guildId, guildName, children }: { guildId: string; guildName: string; children: React.ReactNode }) {
  ...
  return <ThemeScope><div className="relative flex min-h-screen flex-col overflow-x-clip bg-[#070a0f] text-white selection:bg-cyan-400/30 [scrollbar-gutter:stable]" style={{ backgroundImage: "linear-gradient(rgba(7,10,15,.94), rgba(7,10,15,.98)), url('/public-minecraft-ambience.png')", backgroundSize: 'cover', backgroundAttachment: 'fixed' }}><div className="pointer-events-none fixed inset-0 opacity-40 [background-image:linear-gradient(rgba(255,255,255,.025)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.025)_1px,transparent_1px)] [background-size:48px_48px]" /><div className="pointer-events-none fixed -left-32 top-40 size-96 rounded-full bg-cyan-400/[0.05] blur-3xl" /><div className="pointer-events-none fixed -right-40 bottom-20 size-[28rem] rounded-full bg-amber-300/[0.04] blur-3xl" />
    ...
    <footer className="relative z-10 mt-auto shrink-0 border-t border-white/[0.08] bg-[#05080c] px-5 py-5 sm:px-8"><div className="mx-auto flex max-w-[95rem] items-center justify-between gap-4 text-[10px] text-white/35"><span className="truncate">{guildName} <span className="text-white/15">·</span> Competitive community portal</span><span className="shrink-0 font-mono">{guildDomain(guildId)}</span></div></footer>
  </div></ThemeScope>
}

export const publicIcons = { Users, Trophy, ShoppingBag, Gamepad2, Activity }
export function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) { return <div className="public-enter mb-10 flex flex-col items-center text-center"><p className="mb-3 font-mono text-[11px] font-bold tracking-[0.24em] text-cyan-300">// {eyebrow}</p><h1 className="text-4xl font-black tracking-[-0.04em] sm:text-5xl">{title}</h1><p className="mt-4 max-w-2xl text-base leading-7 text-white/55">{description}</p></div> }
export function PublicCard({ children, className = "", id }: { children: React.ReactNode; className?: string; id?: string }) { return <div id={id} className={`public-card min-w-0 rounded-xl border border-white/[0.09] bg-white/[0.025] shadow-[0_18px_48px_rgba(0,0,0,.12)] ${className}`}>{children}</div> }
