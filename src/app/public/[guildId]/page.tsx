...
export default async function PublicHome({
	...
	const podium = [
		{ player: leaders[1], position: 2 },
		{ player: leaders[0], position: 1 },
		{ player: leaders[2], position: 3 },
	];
		...
		<div>
			<section className="relative overflow-hidden border-b border-white/[0.06]">
				...
				<div className="relative mx-auto grid max-w-[95rem] gap-12 px-5 pb-20 pt-20 sm:px-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:pb-28 lg:pt-28">
					...
					<div>
						<div className="mb-6 inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/[0.06] px-3 py-1.5 font-mono text-[10px] tracking-wider text-cyan-200">
							<Zap className="size-3" /> SEASON 14 · LIVE
						</div>
					...
					<PublicCard className="relative overflow-hidden p-6 sm:p-7">
						...
						<div className="mb-7 flex items-center justify-between">
							<span className="font-mono text-[10px] tracking-[.2em] text-white/45">
								COMMUNITY STATUS
							</span>
			...
			<section className="mx-auto max-w-[95rem] px-5 pb-10 pt-16 sm:px-8 sm:pt-20">
				<SectionHeading
					eyebrow="THE SEASON BOARD"
					title="The names setting the pace."
					description="Track the players, matches, and moments defining the current climb."
				/>
				<div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
					{publicStats.map(([label, value]) => (
						...
						>
							<p className="font-mono text-[9px] tracking-[.18em] text-white/35">
								{label}
							</p>
				...
				<div className="mx-auto mt-12 grid max-w-5xl items-end gap-4 md:grid-cols-3">
					{podium.map(
						({ player, position }) =>
							player && (
								<Link key={player.id} href={`${base}/players/${player.slug}`}>
									<PublicCard
									...
									>
										...
										<img
											src={`https://nmsr.nickac.dev/face/${encodeURIComponent(player.name)}`}
			...
			<section className="mx-auto max-w-[95rem] px-5 pb-14 sm:px-8">
				<div className="mb-4 flex items-center justify-between">
					<p className="font-mono text-[10px] tracking-[.2em] text-white/30">
						EXPLORE THE NETWORK
					</p>
					<span className="h-px flex-1 bg-white/[0.08] ml-4" />
				</div>
				<div className="grid gap-4 md:grid-cols-3">
					{[
						[
							"Players",
							"Browse competitors and find the names shaping the season.",
							Users,
							"/players",
						],
						[
							"Games",
							"Track recent matches, modes, maps, and results.",
							Gamepad2,
							"/games",
						],
						[
							"Leaderboard",
							"See who is leading the network across every ranked queue.",
							Trophy,
							"/leaderboard",
						],
			...
			<section className="mx-auto grid max-w-[95rem] gap-4 px-5 pb-24 sm:px-8 md:grid-cols-2 lg:grid-cols-4">
				{[
					[
						"About",
						"/about",
						"A fair, transparent place to queue, compete, and track every step of the climb.",
					],
					[
						"Creators",
						"/creators",
						"Meet the players, streamers, and community voices keeping the competition moving.",
					],
					[
						"Partners",
						"/partners",
					...
					[
						"Support",
						"/support",
						"Find answers, community guidance, and the right place to ask.",
					],
				].map(([title, href, copy]) => (
