...
export default function Page() {
	...
	const [partyLimits, setPartyLimits] = useState({ ...DEFAULT_PARTY_LIMITS });
	const [savedPartyLimits, setSavedPartyLimits] = useState({ ...DEFAULT_PARTY_LIMITS });

	const { isDirty } = useUnsavedChanges(partyLimits, savedPartyLimits);

	const handleInputChange = (field: keyof typeof partyLimits, value: unknown) => {
	...
	const handleSaveChanges = () => {
		setIsSaving(true);
		setTimeout(() => {
			setSavedPartyLimits({ ...partyLimits });
			setIsSaving(false);
		}, 900);
	};
