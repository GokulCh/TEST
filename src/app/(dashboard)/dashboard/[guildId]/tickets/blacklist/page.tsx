...
export default function Page() {
	...
	const updateEntry = (id: string, updates: Partial<BlacklistEntry>) => {
		setBlacklist((prev) => prev.map((item) => (item.id === id ? { ...item, ...updates } : item)));
	};
							...
							<tbody className="divide-y divide-border-subtle/40">
								{filteredBlacklist.map((item) => (
									<tr key={item.id} className="hover:bg-panel-bg/10 transition-colors">
										<td className="p-4 font-bold text-fg-default">
											<input
												type="text"
												value={item.user}
												onChange={(e) => updateEntry(item.id, { user: e.target.value })}
												placeholder="username"
												className="bg-transparent font-bold text-fg-default focus:outline-none w-full placeholder:text-fg-muted/40"
											/>
										</td>
										<td className="p-4 text-fg-muted max-w-[200px]">
											<input
												type="text"
												value={item.reason}
												onChange={(e) => updateEntry(item.id, { reason: e.target.value })}
												placeholder="reason"
												className="bg-transparent text-fg-muted focus:outline-none w-full placeholder:text-fg-muted/40 truncate"
											/>
										</td>
										<td className="p-4">
											<select
												value={item.duration}
												onChange={(e) => updateEntry(item.id, { duration: e.target.value })}
												className="bg-transparent font-bold text-rose-400 focus:outline-none cursor-pointer"
											>
												<option>1 Day</option>
										...
										<td className="p-4 text-fg-muted">
											<input
												...
												onChange={(e) => updateEntry(item.id, { assignedBy: e.target.value })}
												placeholder="staff"
												className="bg-transparent text-fg-muted focus:outline-none w-full placeholder:text-fg-muted/40"
											/>
										</td>
										...
										<td className="p-4 text-right">
											<button
												...
												className="size-7 flex items-center justify-center border border-border-subtle hover:bg-red-500/10 text-fg-muted hover:text-red-400 rounded-md transition-all cursor-pointer ml-auto"
											>
