...
export default function Page() {
	...
	const [embedState, setEmbedState] = useState<EmbedState>({ ...DEFAULT_EMBED_STATE, presets: { ...DEFAULT_PRESETS } });
	const [savedEmbedState, setSavedEmbedState] = useState<EmbedState>({ ...DEFAULT_EMBED_STATE, presets: { ...DEFAULT_PRESETS } });

	const { isDirty } = useUnsavedChanges(embedState, savedEmbedState);
	...
	const handleSaveChanges = () => {
		setIsSaving(true);
		setTimeout(() => {
			setSavedEmbedState(JSON.parse(JSON.stringify(embedState)));
			setIsSaving(false);
		}, 900);
	};
