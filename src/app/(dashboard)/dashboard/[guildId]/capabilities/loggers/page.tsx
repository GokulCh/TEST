...
export default function Page() {
	...
	const [loggers, setLoggers] = useState<LoggerRecord[]>([]);
	const [savedLoggers, setSavedLoggers] = useState<LoggerRecord[]>([]);

	const { isDirty } = useUnsavedChanges(loggers, savedLoggers);

	const handleSaveChanges = () => {
		setIsSaving(true);
		setTimeout(() => {
			setSavedLoggers(JSON.parse(JSON.stringify(loggers)));
			setIsSaving(false);
		}, 900);
	};
	...
	const updateLogger = (id: string, updates: Partial<LoggerRecord>) => {
		setLoggers((prev) => prev.map((l) => (l.id === id ? { ...l, ...updates } : l)));
	};
						...
						>
							<div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
								<div className="space-y-1 sm:col-span-2">
									...
									<select
										value={l.category}
										onChange={(e) => updateLogger(l.id, { category: e.target.value })}
								...
								<div className="space-y-1">
									...
									<input
										type="text"
										value={l.channel}
										onChange={(e) => updateLogger(l.id, { channel: e.target.value })}
