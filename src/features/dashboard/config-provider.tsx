		...
		fetch(`/api/guilds`)
			...
			.then(async (d) => {
				...
				const [configRes, metaRes, queueRes] = await Promise.allSettled([
					fetch(`/api/db/guilds/${resolvedDbId}/config`).then((r) => r.json()),
					fetch(`/api/db/guilds/${resolvedDbId}/meta`).then((r) => r.json()),
					fetch(`/api/db/guilds/${resolvedDbId}/matchmaking`).then((r) => r.json()),
				]);
